    ...
    return new Promise((resolve, reject) => {
        state.stream = ffmpeg()
            ...
            .on('progress', (progress) => {
                io.emit('progress', { percentage: progress.percent, message: `Merging`, showPercentage: true })

            })
            .on('end', () => {
                resolve();
                io.emit('progress', { percentage: 100, message: `Done`, showPercentage: true })
            })
...
async function downloadContent(props) {
    ...
    stream.on("progress", (_, current, total) => {
        ...
        io.emit('progress', { percentage, message: `${props.updateMessage}`, showPercentage: true })
    });

    return new Promise((resolve, reject) => {
        stream.on("finish", () => {
            io.emit('progress', { percentage: null, message: `Working`, showPercentage: false })
            state.downloading = false;
            state.stream = null;
            resolve();
